const selectedPlaces = [
  {
    ...
    treasureHint: "보물힌트: 완주군정신보건센터 표지판 아래",
  ...
  { id: "sanggwan-forest", title: "상관역", query: "상관역 완주", intro: "완주 상관면으로 이어지는 철도 교통 거점입니다.", fallback: [35.7676, 127.2055] },
  {
    ...
    treasureHint: "보물힌트: 버스정류장 출입문 근처",
  ...
  { id: "woosuk-hospital", title: "우석대학교 부속 전주한방병원", query: "우석대학교 부속 전주한방병원", intro: "지역 의료·건강 테마와 연결할 수 있는 도심 거점입니다.", treasureHint: "보물힌트: 유리창 맨 아래", fallback: [35.8137, 127.0892] },
  { id: "sebyeongho", title: "세병호", query: "세병호 전주", intro: "전주 북부권의 수변 경관을 즐길 수 있는 호수형 방문 장소입니다.", treasureHint: "보물힌트: 네이버 밴드내용 참조", fallback: [35.8816, 127.1461] },
  {
    id: "yeoyu-cafe",
    title: "무인카페여유",
  ...
  { id: "daedeok-elementary", title: "대덕초등학교", query: "대덕초등학교 완주", intro: "지역 생활권을 연결하는 교육 시설 주변 위치 표시 지점입니다.", treasureHint: "보물힌트: 정문 입구", fallback: [35.9139, 127.2386] },
  { id: "chucheondae", title: "추천대", query: "추천대 완주", intro: "완주권 역사·경관 동선과 연결할 수 있는 방문 거점입니다.", fallback: [35.9589, 127.2656] },
  { id: "deokjin-park", title: "덕진공원", query: "덕진공원", intro: "전주 대표 공원으로 수변 경관과 휴식 동선이 좋은 시민 친화형 장소입니다.", treasureHint: "보물힌트: 현판 안, 72번 가로등 기둥(후문)", fallback: [35.8482, 127.1201] },
  {
    id: "volunteer-center",
    title: "전주시 자원봉사센터",
    query: "전주시자원봉사센터",
...
function getPlacePhotoPageUrl(place) {
...
function createPlacePhotoMarkup(place) {
  ...
  if (!photoPageUrl) {
    return `<figure class="place-card-photo">${imageMarkup}</figure>`;
  }

  return `
    <figure class="place-card-photo">
      <a href="${escapeHtml(photoPageUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(`${place.title} 참고 이미지 새 창으로 열기`)}">
        ${imageMarkup}
      </a>
    </figure>
  `;
...
function handleRouteSearchSubmit(event) {
  ...
  renderPlaceSearchResults(results, selected.id);
  elements.routeSearchActions.hidden = true;
...
function renderPlaceSearchResults(results, selectedPlaceId) {
  if (!elements.routeSearchResults) {
    return;
  }

  elements.routeSearchResults.innerHTML = `
    <p class="route-search-results-title">검색 결과</p>
    <div class="route-search-result-list">
      ${results
        .slice(0, 6)
        .map(
          ({ place }) => `
            <button class="route-search-result${place.id === selectedPlaceId ? " is-active" : ""}" type="button" data-search-place="${escapeHtml(place.id)}">
  ...
  elements.routeSearchResults.querySelectorAll("[data-search-place]").forEach((button) => {
    button.addEventListener("click", () => {
      state.routeSearchDestination = getPlaceById(button.dataset.searchPlace);
      focusPlace(button.dataset.searchPlace);
      elements.routeSearchResults
        .querySelectorAll(".route-search-result")
